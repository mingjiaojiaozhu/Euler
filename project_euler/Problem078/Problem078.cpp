...
private:
    ...
    Decimal *get_way(int target, vector<Decimal *> &ways) {
    ...
    bool add_decimal(int index, vector<Decimal *> &ways, int factor, Decimal *result) {
        if (index < 0) {
            return false;
        }
        get_summation(ways[index], factor, result);
        return true;
    }
